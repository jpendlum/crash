...
int main()
{
    ...
    while(kill_prog == 0) {
        ...
        uint32_t reset_on_rx_tlast = 0;                 // Kludge to prevent RX FIFO overflow due to slow FFT
        if (decim == 1 || decim == 2) {
            reset_on_rx_tlast = 1;
        }
        ...
        WRITE_REG(spectrum_sense_reg[1],nfft_log,0,5);          // Set FFT Size 4096
        ...
        SET_BIT(spectrum_sense_reg[1],13);                      // Enable clear threshold latched
        ...
        WRITE_REG(usrp_ddr_intf_reg[3],decim,0,13);             // Set decim = 1
        WRITE_REG(usrp_ddr_intf_reg[4],gain,0,25);              // Set gain = 1
        SET_BIT(usrp_ddr_intf_reg[0],0);                        // Enable RX
        SET_BIT(usrp_ddr_intf_reg[0],3);                        // Enable TX Sideband

        // The idea is to inject a RF pulse that has a high duty rate and a long period.
        // For this test, we used a 5 second pulse with 500 usec "off cycle". The code
        // below first waits until we detect the pulse's on cycle, then waits until the
        // pulse's off cycle, then waits AGAIN for the pulse's on cycle, and
        // finally we setup CRASH to transmit on the next off cycle. This process ensures
        // CRASH is completely setup and spectrum sensing before the off cycle. If we did
        // not follow this process, it would be possible that we begin spectrum sensing in
        // the middle of an off cycle which will give us false turnaround time.

        // Wait until FFT Threshold is exceeded (or Ctrl-C)
        while(GET_BIT(spectrum_sense_reg[3],31) == 0 && kill_prog == 0);
        //CLEAR_BIT(spectrum_sense_reg[1],10);                    // Disable IRQ on threshold exceeded

        // Pointer games to extract floats from registers
        float *fpga_threshold = (float*)(&spectrum_sense_reg[2]);
        float threshold_print = *fpga_threshold;
        float *fpga_threshold_mag = (float*)(&spectrum_sense_reg[4]);
        float threshold_mag_print = *fpga_threshold_mag;
        unsigned int threshold_index_print = READ_REG(spectrum_sense_reg[3],0,16);

        // Print threshold from earlier
        printf("Threshold:\t\t\t%f\n",threshold_print);
        printf("Threshold Exceeded Index:\t%d\n",threshold_index_print);
