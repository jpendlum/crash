...
int main()
{
    ...
    while(1) {
        ...
        // Setup Receive Path
        WRITE_REG(usrp_ddr_intf_reg[0],0,29,3);                 // Set RX destination to Spectrum Sense
        ...
        WRITE_REG(usrp_ddr_intf_reg[4],gain,0,32);              // Set gain = 1
        ...
        // Setup FFTW3
        fftwf_complex *in1 = (fftw_complex *)(&buffer[4096]);
        fftwf_complex out[nfft];
        fftwf_plan p1 = fftwf_plan_dft_1d(nfft, in1, out, FFTW_FORWARD, FFTW_ESTIMATE);
        ...
        while (runs == 0) {
            ...
            //runs = 1;
            for (k = 0; k < nfft; k++) {
                //printf("%04d : %f : %f\n",k,sqrt(out[k][0]*out[k][0] + out[k][1]*out[k][1]));
                if (sqrt(out[k][0]*out[k][0] + out[k][1]*out[k][1]) > threshold) {
                    SET_BIT(usrp_ddr_intf_reg[0],1);        // Enable TX
                    stop = READ_REG(ps_pl_interface_reg[11],0,32);
                    threshold_exceeded_mag = sqrt(out[k][0]*out[k][0] + out[k][1]*out[k][1]);
                    runs = 1;
                    break;
                }
            }
