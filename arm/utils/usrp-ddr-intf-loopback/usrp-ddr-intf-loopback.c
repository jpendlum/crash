...
int main()
{
    ...
    // Setup Transmit Path
    CLEAR_BIT(usrp_ddr_intf_reg[2],24);                       // Bypass float2fix
    WRITE_REG(usrp_ddr_intf_reg[3],1,16,13);                // Set interp = 1
    WRITE_REG(usrp_ddr_intf_reg[5],(1 << 4),0,25);                 // Set gain = 1

    // Setup Receive Path
    WRITE_REG(usrp_ddr_intf_reg[0],1,29,3);                 // Set RX destination to itself
    WRITE_REG(usrp_ddr_intf_reg[2],4096,0,23);              // Set packet size
    CLEAR_BIT(usrp_ddr_intf_reg[2],23);                     // Do not bypass fix2float
    WRITE_REG(usrp_ddr_intf_reg[3],4,0,13);                 // Set decim = 1
