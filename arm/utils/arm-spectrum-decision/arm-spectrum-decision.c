...
#define TX_RX_LOOPBACK_MODE     (0x4 << 4)
...
// Struct to use when our 64-bit buffer data is a composed on a float & int
typedef struct {
    float    f;
    uint32_t i;
} uint_float;
...
int main()
{
    ...
    // Setup structs for nanosleep
    struct timespec ask_sleep, act_sleep;
    ask_sleep.tv_sec = 2;
    ask_sleep.tv_nsec = 0;
    ...
    while (kill_prog == 0) {
        ...
        WRITE_REG(global_reg[1],15,3,4);    // ARCACHE: "1111"
        WRITE_REG(global_reg[1],31,7,5);    //  ARUSER: "11111"
        WRITE_REG(global_reg[1],15,15,4);   // AWCACHE: "1111"
        ...
        // Wait for awhile so TX buffers fill
        ...
        uint32_t nfft_log = (uint32_t)log2((double)nfft);
        uint32_t t;
        memcpy(&t,&threshold,sizeof(float));            // Copy float value to an int without a cast
        ...
        WRITE_REG(usrp_ddr_intf_reg[5],1,0,32);     // Set gain = 1
        SET_BIT(usrp_ddr_intf_reg[0],3);            // Enable TX Sideband

        // Setup Spectrum Sensing
        WRITE_REG(spectrum_sense_reg[1],fft_mode,8,2);  // FFT Mode
        SET_BIT(spectrum_sense_reg[0],0);               // Enable FFT
        WRITE_REG(spectrum_sense_reg[1],nfft_log,0,5);  // Set FFT Size
        SET_BIT(spectrum_sense_reg[1],5);               // Set FFT Size Enable
        CLEAR_BIT(spectrum_sense_reg[1],5);             // Clear FFT Size Enable
        //SET_BIT(spectrum_sense_reg[1],11);              // Enable sideband threshold exceeded output (to trigger TX)
        //SET_BIT(spectrum_sense_reg[1],10);              // Enable threshold detect IRQ
        WRITE_REG(spectrum_sense_reg[2],t,0,32);        // Threshold level in single precision floating point
        ...
        WRITE_REG(ps_pl_interface_reg[5],nfft*8,0,23);              // Number of bytes to send
        WRITE_REG(ps_pl_interface_reg[5],2,23,3);                   // Set master / accelerator to transfer from (tid)
        //SET_BIT(ps_pl_interface_reg[5],31);                         // Push CMD to S2MM CMD FIFO

        // Receive enable, bypass fix2float and decim = 1 and gain = 1
        WRITE_REG(usrp_ddr_intf_reg[0],2,29,3);                     // Set RX destination to Spectrum Sense
        WRITE_REG(usrp_ddr_intf_reg[2],nfft,0,23);                  // Set packet size
        CLEAR_BIT(usrp_ddr_intf_reg[2],23);                         // Do not bypass fix2float
        WRITE_REG(usrp_ddr_intf_reg[3],decim,0,13);                 // Set decim
        WRITE_REG(usrp_ddr_intf_reg[4],gain,0,32);                  // Set gain
        SET_BIT(usrp_ddr_intf_reg[0],0);                            // Enable RX

        uint_float *sample = (uint_float *)&buffer[4096];
        float threshold_exceeded_mag;
        int runs = 0;
        int k = 0;
        int dummy = 0;
        uint32_t start;
        ...
        uint32_t start_overhead_xfer;
        uint32_t stop_overhead_xfer;

        start_overhead_xfer = READ_REG(ps_pl_interface_reg[11],0,32);
        stop_overhead_xfer = READ_REG(ps_pl_interface_reg[11],0,32);

        SET_BIT(ps_pl_interface_reg[5],31);                     // Push CMD to S2MM CMD FIFO
        while(GET_BIT(ps_pl_interface_reg[8],2) == 1);
        dummy = READ_REG(ps_pl_interface_reg[7],0,8);

        start_overhead = READ_REG(ps_pl_interface_reg[11],0,32);
        SET_BIT(ps_pl_interface_reg[5],31);                     // Push CMD to S2MM CMD FIFO
        while(GET_BIT(ps_pl_interface_reg[8],2) == 1);
        dummy = READ_REG(ps_pl_interface_reg[7],0,8);
        stop_overhead = READ_REG(ps_pl_interface_reg[11],0,32);
        ...
        while(runs == 0) {
            //while(GET_BIT(ps_pl_interface_reg[8],2) == 1);
            //dummy = READ_REG(ps_pl_interface_reg[7],0,8);
            // Speed trick here. Due to a flaw in the FPGA code, a backup can occur in the
            // usrp_ddr_intf_axis FIFO. This avoids the problem by constantly performing
            // transfers.
            SET_BIT(ps_pl_interface_reg[5],31);                     // Push CMD to S2MM CMD FIFO
            SET_BIT(ps_pl_interface_reg[5],31);                     // Push CMD to S2MM CMD FIFO
            //runs = 1;
            //start = READ_REG(ps_pl_interface_reg[11],0,32);
            for (k = 0; k < nfft; k++) {
                //printf("%4d : %016llx : %016llx : %08x : %08f\n",k,buffer[4096+k] & 0xFFFFFFFF,buffer[4096+k] & 0xFFFFFFFF00000000,sample[k].i,sample[k].f);
                if (sample[k].f > threshold) {
                    SET_BIT(usrp_ddr_intf_reg[0],1);        // Enable TX
                    //stop = READ_REG(ps_pl_interface_reg[11],0,32);
                    runs = 1;
                    threshold_exceeded_mag = sample[k].f;
                    break;
                }
            }
        }
        printf("AXI GP:\t\t\t%d\n",stop_overhead_xfer - start_overhead_xfer);
        printf("DMA:\t\t\t%d\n",stop_overhead - start_overhead);
        printf("Calculation:\t\t%d\n",stop - start);
        printf("Threshold:\t\t%f\n",threshold);
        printf("Threshold Exceeded:\t%f\n",threshold_exceeded_mag);
        printf("\n");

        // Finish remaining transfer
        while(GET_BIT(ps_pl_interface_reg[8],2) == 1);
        dummy = READ_REG(ps_pl_interface_reg[7],0,8);
